...
# which returns true/True if argument is string decimal representation of a lucky ticket number, 
...
def luck_check(string):
    left, right = len(string) // 2, (len(string) + 1) // 2
    return sum(map(int, string[:left])) == sum(map(int, string[right:]))